...
test("separate values, legacy fallback, missing profiles and mobile layout", async ({ page }) => {
    ...
    await expect(page.locator('[data-label="ФИО"]')).toHaveText(["Яковлев Иван Иванович", "Борисов Борис", "—", "Антонов"]);
    await expect(page.locator('[data-label="Ник"]')).toHaveText(["alpha", "zeta", "—", "beta"]);
    await expect(page.locator('[data-label="Телефон"]')).toHaveText(["+79990000003", "+79990000001", "—", "+79990000002"]);
...
test("manual creation requires patronymic and rejects whitespace names", async ({ page }) => {
    ...
    await page.getByPlaceholder("Имя", { exact: true }).fill("Иван");
    await page.getByPlaceholder("Отчество", { exact: true }).fill("Иванович");
    await page.getByPlaceholder("Телефон", { exact: true }).fill("+79991234567");
...
test("completion requires typed FIO and sends trimmed name parts", async ({ page }) => {
    ...
    await page.getByPlaceholder("Например, boxer").fill("boxer");
    ...
    await page.getByRole("button", { name: "Сохранить профиль" }).click();
    await expect.poll(() => submitted).toMatchObject({ surname: "Иванов", firstName: "Иван", patronymic: "Иванович", nickname: "boxer" });
});
